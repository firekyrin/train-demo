...
static SEL HHOriginSelector(NSString *selectorName) {
    return NSSelectorFromString([NSString stringWithFormat:@"HH_ORIGIN%@", selectorName]);
}
...
value = [NSValue valueWith##_type:argument]; \
    ...
    if ([selectorName hasPrefix:@"ORIG"]) {
        selectorName = [selectorName substringFromIndex:4];
    }
    [HHObserver object:target willInvokeFunction:selectorName withArguments:arguments];
    [invocation setSelector:HHOriginSelector(selectorName)];
    [invocation invoke];
...
+ (void)replaceClass:(Class)cls function:(NSString *)selectorName {
    ...
    SEL forwardSelector = HHOriginSelector(selectorName);
...
+ (NSDictionary<NSString *, id> *)observeItems {
    return @{@"UIControl": @"sendAction:to:forEvent:",
             ...
             @"SencondViewController": @[@"aFunc",
                                         ...
                                         @"dasidesadbisaidiafaweffwgwfaw"]};
}

+ (void)object:(id)object willInvokeFunction:(NSString *)function withArguments:(NSArray *)arguments {
    NSLog(@"\n--------------\nobject: %@willInvokeFunction: %@\nwithArguments: %@\n------------", object, function, arguments);
}
